...
bool AsteroidPool::removeAsteroid(unsigned ID) {
    if ((ID < 0) || (ID >= m_activeIndex))            // Safeguard against IDs outside of the active pool
        return false;
    ...
    else                                // Swap the tail into the newly vacated index
    {
        ...
        m_Circles[ID].setPointCount(m_Circles[m_activeIndex].getPointCount());
        m_Particles[ID].setMass(m_Particles[m_activeIndex].getMass());
        m_Particles[ID].setDamping(m_Particles[m_activeIndex].getDamping());
        m_Particles[ID].setPosition(m_Particles[m_activeIndex].getPosition());
        m_Particles[ID].setVelocity(m_Particles[m_activeIndex].getVelocity());
        m_Particles[ID].setAcceleration(m_Particles[m_activeIndex].getAcceleration());
    }
    return true;
}
